...
The **Codex** or **Claude** label on account cards, selected details, tray tooltips and the widget identifies the usage provider. Connect the current Claude login or sign in through the official browser flow; CycleArc sets up its **statusLine** connection automatically and reads Claude Desktop subscription usage history when available. See [Claude Code connection](#claude-code-connection). Gemini is not supported.

> **Codex:** five-hour and weekly limits appear when the official App Server reports them, including on Plus; display is not restricted by plan name. Missing windows are omitted and unknown percentages stay unknown. **Claude:** usage can come from the official statusLine or Claude Desktop's subscription usage history; missing windows stay unknown, and Desktop history has no reset timestamps.
...
- **Honest refresh states.** Check Codex at a selectable 1, 2, 5, 10, 30 or 60-minute interval (default: five minutes). Claude shows the latest observed shared subscription quota from Claude Code or Claude Desktop. Previously received values remain visibly stale during a temporary update failure; unconnected profiles stay in account management.

## Get started

**Requirements:** Windows 10/11 on x64. Codex monitoring requires an installed Codex CLI signed into a ChatGPT account that reports subscription limits, and network access. Claude monitoring requires the official Claude CLI for account verification, Windows PowerShell, and a signed-in Claude Pro or Max account. Usage can arrive from Claude Code statusLine or Claude Desktop's subscription usage history; Codex sign-in is not required.
...
4. Codex appears after a successful quota check; Claude appears after verified connection, with unknown limits until a valid Code or Desktop history sample is available. If Codex cannot be found, install the [Codex CLI](https://developers.openai.com/codex/cli/) or open **Settings → Connection** and select its executable path. For Claude setup, follow the [connection steps below](#claude-code-connection).
...
**Claude subscription usage is shared across Web, Desktop and Code.** CycleArc displays the latest shared-quota sample observed from Claude Code statusLine or Claude Desktop's subscription usage history. Web/Desktop activity also consumes that allowance, so the current account usage may have changed since the sample. [Official usage-limit explanation](https://support.claude.com/en/articles/11647753-how-do-usage-and-length-limits-work).

To check current usage without using CycleArc's local sources, choose **Open usage page** on the Claude detail card or connection window. It opens [Claude Settings → Usage](https://claude.ai/settings/usage) in your browser; check that the intended account is signed in. Opening the page does not refresh CycleArc. A review of official APIs, CLI and SDK documentation found no supported public independent query for a personal subscription's current shared quota. See the [dated research and decision](docs/CLAUDE-USAGE-RESEARCH.md).
...
3. During normal **Claude Code terminal (CLI)** use, statusLine responses can supply shared subscription usage. Claude Desktop Code can also write a subscription usage history sample. **Open Claude Code terminal…** launches the terminal with that profile's configuration and your chosen working folder; use it when awaiting a first statusLine sample, especially for a separate login created by CycleArc. CycleArc reads Desktop's app-owned `%LOCALAPPDATA%\Packages\Claude_pzs8sxrjxfjjc\LocalCache\Roaming\Claude\plan-usage-history.json` when available, with the legacy `%APPDATA%\Claude\plan-usage-history.json` path as a fallback. Desktop history is version 2 with `t` (Unix milliseconds), `org`, and `u.fh` / `u.sd` percentages; it has no reset timestamps. A Desktop write may be delayed. Only the official statusLine `rate_limits.five_hour` / `rate_limits.seven_day` → `used_percentage` and `resets_at` fields supply reset-aware usage; absent values stay unknown.
...
**CycleArc supports usage receipt from the connected Claude Code terminal and Claude Desktop's subscription usage history.** Connection and usage receipt are separate: a connected account stays visible with **Awaiting usage** until either source provides a valid sample. After using Desktop Code, wait for its history file to update and choose **Refresh**; CycleArc reads it without starting Desktop or a model request. When both sources are present, the newer observation wins. Repeating **Connect current login** reuses the same verified configuration binding and preserves its name and usage history. Closing or cancelling a new connection before it succeeds removes its empty draft from the list.
...
The official statusLine JSON has **no account email or account ID**. Desktop history carries an organization ID, not an email. The official CLI supplies the current login's email and subscription metadata; CycleArc derives an identity fingerprint for the local binding and accepts Desktop samples only for a matching verified Claude Pro/Max account and organization. Email stays in memory. Each callback and newly observed Desktop sample checks that the current login still matches; a different or unverifiable identity cannot replace the last good sample. This checks the configured login, not the identity of an already-running session, so restart existing Claude sessions after switching their login outside CycleArc. Browser logins started by CycleArc use separate configuration folders. A nickname takes precedence over the verified login email, with `Claude · <local profile ID>` as the fallback. Account selection, order and aliases share the existing UI; profiles are never added together.

The last valid sample stays **Received** while idle, including after a reported reset time passes, with its original **Last received** date and time. Elapsed time does not raise a warning or reset the saved percentage to zero. **Stale data** appears only when input is missing/malformed, the Desktop history is unavailable or changed, or receipt/cache/identity checks fail. Those warnings use amber text on the detail card, account cards and widget, and an amber detail ring. Percentages and receipt timestamps remain visible through restarts and in the tray tooltip. The shared Web·Desktop·Code scope stays visible because a received sample does not establish current server usage. Manual refresh and passive local reads do not renew receipt time, query Claude, start Desktop, or synthesize a new quota period. Desktop history has no reset timestamps; those remain unknown unless a newer statusLine sample supplies them. Claude has no Codex reset-credit controls.
...
**Connection details → Disconnect** first saves the disconnected state, then restores the previous status line and removes CycleArc's own failure hook. The profile stays hidden from the main view across restarts even if the settings file cannot be cleaned up; CycleArc reports that cleanup is incomplete. The profile stays in account management, with its last good cache and configuration location preserved. Reconnecting shows the account with **Awaiting usage** until a new statusLine or Desktop history sample arrives; it does not log out of Claude.
...
Usage comes from the official statusLine input or Claude Desktop's app-owned local history. Authentication uses the official `claude auth login --claudeai` and `claude auth status --json` commands. CycleArc does not parse `/usage`, read Claude auth/token/cookie files, launch a model turn, inspect transcripts or call an undocumented usage endpoint. It saves projected quota fields, source/receipt/status metadata and local connection paths/fingerprint, never the full stdin or Desktop history JSON. See [integration details](docs/CLAUDE.md), the [Desktop research notes](docs/CLAUDE-USAGE-RESEARCH.md) and the [official statusLine documentation](https://code.claude.com/docs/en/statusline).
...
Percentages come from the reported limit windows. CycleArc does **not** turn them into invented request counts or combine unrelated reset periods. A temporary refresh failure can leave the last verified snapshot visible with a stale label. After restart, Codex must verify the profile's login before cached values can be shown; a changed or unverifiable identity has no displayed quota or usable reset credits. Opening the card immediately after a failed check does not trigger repeated automatic retries; manual refresh remains available. Claude uses an independent passive provider: statusLine stdin is received through a headless mode of the same executable, while Desktop history is read from its app-owned file. Both local sources are checked every two seconds without starting Codex or a model request; Desktop attribution runs the bounded official auth-status command on startup, binding changes and newly observed samples, then reuses that verified identity for unchanged samples. The newer observation is displayed.
...
- No direct reading or copying of Codex or Claude authentication files, tokens, browser cookies, or credentials. Claude Desktop's app-owned usage-history file is read only for projected quota samples.
- No external telemetry or analytics.
- Only local preferences, profile references/labels, projected quota metadata, source/receipt metadata, connection paths/identity fingerprints and safe diagnostic logs are retained. Reported emails stay in memory for display. Authentication is handled by the installed Codex or Claude CLI; Desktop history is never copied into CycleArc's cache.
...
CycleArc is an independent project and is not affiliated with or endorsed by OpenAI or Anthropic. Compatibility depends on the installed Codex App Server, Claude CLI/statusLine protocol, Claude Desktop history schema and the metadata available to your account.
