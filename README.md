...
The **Usage number** tray style shows bold digits in their original font proportions with a smaller **%** (for example, **67%**) on a transparent background. Text is white on a dark Windows taskbar and dark on a light taskbar, independently of the app theme. The number is the selected account's percentage used, preferring weekly usage when known and otherwise five-hour usage. Unknown usage is **?**. Check the tooltip or detail card for status and receipt time. The optional **Usage ring** retains its status colors: blue for a valid sample, red for a valid sample at 100%, and amber for stale data or a lookup/connection problem. Without a known value, the ring is gray with **?**.
