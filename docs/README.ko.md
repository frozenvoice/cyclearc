...
Cursor는 두 트레이 아이콘 스타일의 숫자를 정수로 반올림하여 **76.91% → 77**처럼 표시합니다. 위젯에 보이는 퍼센트도 사용 **76.91% → 77%**, 잔여 **23.09% → 23%**처럼 반올림하며 금액 표시는 유지합니다. 툴팁·상세 팝업에는 소수점을 유지하고, 원본 수치·계산·링 진행률·경고 색상은 바꾸지 않습니다. 미확인값은 **?**로 남기며 Codex·Claude 표시도 그대로 유지합니다.
