...
위젯이나 상세 팝업을 화면 가장자리 근처에 내려놓으면 크기는 유지한 채 8 DIP 안쪽으로 정렬합니다. **Shift를 누른 채 놓으면** 이번 이동의 정렬을 생략합니다. 붙인 뒤 확대·축소하거나 내용 높이가 바뀌어도 해당 여백을 유지합니다. **설정 → 일반 → 화면 가장자리에 자동 정렬**에서 끌 수 있습니다.
