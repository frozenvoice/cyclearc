...
트레이의 **사용률 숫자**는 투명 배경에 글꼴의 원래 비율을 유지한 굵은 숫자와 작은 **%**를 붙여 **67%**처럼 표시합니다. 앱 테마와 별개로 Windows 작업표시줄이 어두우면 흰색, 밝으면 어두운 글자를 사용합니다. 선택한 계정의 사용률이며, 주간 값이 있으면 주간 기준, 없으면 5시간 기준입니다. 알 수 없는 값은 **?**로 표시합니다. 상태와 수신 시각은 툴팁이나 상세 카드에서 확인할 수 있습니다. 선택 가능한 **사용률 링**은 기존 상태 색상을 유지합니다. 파랑은 정상 수신값, 빨강은 정상 수신값의 사용률 100%, 노랑은 이전 데이터 또는 조회·연결 문제입니다. 알 수 있는 값이 없으면 회색 링에 **?**를 표시합니다.
