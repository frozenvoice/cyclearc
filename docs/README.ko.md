...
Windows 트레이에서 **여러 Codex·Claude·Cursor 프로필의 사용률, 남은 비율, 리셋 시각**을 함께 확인하는 앱입니다.

선택한 계정은 상세 카드와 트레이에 표시되며, 위젯은 여러 계정을 나란히 비교하면서 선택한 계정을 강조합니다.

계정 카드·선택한 상세 화면·트레이 툴팁·위젯의 **Codex·Claude·Cursor 표시**로 서비스를 구분합니다. Claude는 현재 로그인을 연결하거나 공식 브라우저 로그인을 시작하면 CycleArc가 Desktop 계정과 바인딩하고, 수동·예약 새로고침에서 공유 한도를 읽기 전용으로 확인합니다. statusLine과 Desktop 구독 사용량 기록은 로컬 기록으로 유지합니다. Cursor는 [이 PC의 현재 로그인](#cursor-연결)을 연결합니다. Gemini는 지원하지 않습니다. 기존 Codex 계정, 설정과 캐시는 유지합니다.
...
Codex 계정 카드에는 제공된 한도별 사용률·잔여 비율을, 상세 화면에는 리셋 시각도 함께 표시합니다. 링·트레이·위젯은 하나의 표시 기간을 공유합니다. 기본 **자동**은 확인 가능한 5시간 값을 우선하고, 없으면 주간 값을 사용합니다. 상세 원그래프 위의 **자동 / 5시간 / 주간**에서 선택하거나, 두 값이 모두 있으면 원그래프를 눌러 전환할 수 있습니다. 세 화면에 즉시 반영되며 재시작 후에도 선택을 유지합니다. 선택한 기간의 값이 없으면 확인 가능한 다른 기간을 표시하고 상세 카드에 이유를 안내합니다. 각 링은 다른 한도나 계정의 가장 높은 사용률이 아니라 자신이 표시하는 한도의 실제 사용률로 색이 정해집니다. 70% 미만은 파랑, 70% 이상은 앰버, 85% 이상은 주황, 100% 이상은 빨강이며 선택 여부와 관계없이 같습니다. 툴팁과 화면 읽기 프로그램에는 **주의**, **소진 임박**, **소진**이 덧붙습니다. 숫자·텍스트·선택 테두리 색은 그대로이고, 이전 데이터와 알 수 없는 값은 기존 표시를 유지합니다.

## Cursor 연결

1. Windows Cursor 앱에서 사용할 계정으로 로그인하세요.
2. CycleArc의 **계정 관리 → 계정 추가 → Cursor 연결**을 누르세요. 토큰을 복사할 필요가 없습니다.
3. 수동 새로고침과 설정된 자동 확인 주기에 서버 한도를 조회합니다. 팝업에 각 한도의 잔여량, 제공된 리셋 시각과 마지막 확인 시각을 표시하고, 위젯에는 간결한 요약과 자세한 툴팁을 제공합니다.

**Cursor Models · 월간**, **Other Models · 월간**, **Grok Bot · 주간**으로 표시하며 이름은 한국어에서도 공식 영문 표기를 유지합니다. 계정 카드·팝업·트레이·위젯에서 한도와 주기를 구분하고, 큰 링에는 어느 한도의 사용률인지 표시합니다. 제공되는 추가 사용 예산도 별도로 유지합니다. API에 없는 한도·리셋 시각은 미확인으로 유지하며, 예전 달러 사용액을 모델별 퍼센트와 합산하지 않습니다. 조회 실패 시 마지막 정상값과 확인 시각을 오래된 데이터로 유지합니다. Cursor 로그인 계정이 바뀌면 기존 계정의 잔여량은 숨깁니다. 해당 프로필에 다시 연결하려면 Cursor에서 원래 계정으로 로그인하세요.

Cursor 위젯은 **Cursor Models → Other Models → Grok Bot** 순서로 제공된 한도를 최대 세 개까지 표시하며 월간·주간으로 묶습니다. 링 안의 사용률 위에 **Cursor**, **Other**, **Grok Bot**으로 대상을 표시하고, 옆의 한도 목록과 툴팁에는 전체 이름을 유지합니다. 링은 응답 순서와 관계없이 같은 우선순위에서 확인 가능한 사용률을 고릅니다. 꺼진 온디맨드와 다른 예산 항목, 정확한 리셋·업데이트 시각은 툴팁과 상세 팝업에서 확인할 수 있습니다. 정상 서버 조회값은 하단 상태 행을 접고, 미확인값·오래된 데이터·인증 오류는 계속 명시합니다.

Grok Bot만 확인할 수 없으면 월간 한도는 정상 갱신 상태로 계속 표시하고 조회합니다. Grok Bot의 재시도 대기는 Grok Bot에만 적용하며, 확인하지 못한 값은 생략하고 팝업에 이유를 안내합니다. 계정 불일치 상태에서도 팝업을 다시 열거나 CycleArc를 재시작할 때 설정된 자동 확인 주기를 지킵니다.

CycleArc는 Cursor의 로컬 계정 저장소에서 필요한 액세스 토큰 하나만 읽어 Cursor 서버 조회에 메모리에서 사용합니다. 토큰을 저장·기록·갱신하지 않고 Cursor 설정을 변경하지 않습니다. 계정 연결 해제는 CycleArc에만 적용됩니다. 기본 Windows 사용자 데이터 경로를 지원하며, 별도 `--user-data-dir` 경로는 자동 검색하지 않습니다. 내부 조회 방식은 변경될 수 있습니다. [Windows 검증과 한도 의미](CURSOR.md).

| Cursor 팝업 | Cursor 위젯 |
| --- | --- |
| ![한도별로 표시하는 Cursor 팝업](images/cursor-popup-ko-light.png) | ![갱신 시각을 표시하는 Cursor 위젯](images/cursor-widget-ko-light.png) |

실제 화면에 모의 사용량을 넣은 미리보기이며, 실제 계정 정보가 아닙니다.
...
<table>
  ...
  <tr>
    <td><img src="images/claude-live-ko-dark.png" alt="업데이트됨 상태와 5시간 46%, 주간 11%, 리셋 시각을 보여주는 합성 Claude 서버 조회" width="440"></td>
    <td><img src="images/claude-live-ko-light.png" alt="업데이트됨 상태와 리셋 시각을 보여주는 합성 Claude 서버 조회 라이트 화면" width="440"></td>
  </tr>
</table>
...
설치 화면에서 위치를 확인하고 **설치**를 누르면 진행 상태가 표시됩니다. 완료되면 **CycleArc 실행**을 선택한 상태로 **마침**을 누르세요.
...
`Ctrl + +` / `Ctrl + -`로 표시 크기를 10%씩 조절합니다(80~150%). 헤더 오른쪽의 `-` · `+` 버튼도 같은 역할을 합니다.
`Ctrl + 0`은 기본 크기로 복원하며 숫자패드도 지원합니다. 선택한 크기는 저장됩니다.
상세 카드와 위젯은 **각자의 크기**를 따로 기억합니다. 단축키와 버튼은 그때 키보드를 가진 창 하나에만 적용되며,
다른 창의 크기를 바꾸지 않습니다. 트레이 메뉴의 **크기 초기화 (100%)** · **위젯 크기 초기화 (100%)**로 각각 되돌릴 수 있습니다.
글자가 겹치지 않도록 팝업 전체가 함께 확대·축소되며, 화면보다 커지지 않게 제한됩니다.

위젯은 표시 가능한 Codex·Claude·Cursor 계정을 계정 관리 순서 그대로 고정 폭의 작은 모듈로 나란히 보여주고 이름과 링을 같은 높이로 정렬합니다. Codex·Claude는 제공된 모든 기간의 남은 한도와 리셋까지 남은 시간을, Cursor는 위에서 설명한 최대 세 한도의 요약을 표시합니다. 계정 사용량을 합산하지 않으며 사용량 순으로 재정렬하지도 않습니다. 한 줄에는 위젯이 놓인 모니터의 작업 영역이 허용하는 만큼만 배치하고 나머지는 다음 줄로 넘깁니다. 작업 영역보다 높아지면 위젯 안에서만 세로로 스크롤합니다.

위젯의 빈 곳이나 헤더를 드래그하면 위치가 저장되고, 드래그가 끝났다고 상세 카드가 열리지는 않습니다. 계정 모듈을 짧게 클릭하면 그 계정을 선택하고 상세 카드를 엽니다. 클릭만으로 새 로그인이나 사용량 조회를 시작하지는 않습니다. 헤더의 `-` · `+` 버튼은 위젯 크기만 조절하고, 새로고침·설정·닫기 버튼은 각각 즉시 확인, 설정 창 열기, 위젯만 숨기기를 합니다(CycleArc는 계속 실행됩니다). 헤더나 위젯의 빈 곳을 짧게 클릭하면 위젯이 키보드 입력을 받을 뿐, 상세 카드가 열리지는 않습니다. 그래서 위젯을 누른 뒤 `Ctrl + +`를 누르면 위젯이 커집니다. 우클릭 → 위젯 닫기로 숨기고 설정에서 다시 켤 수 있습니다. 클릭 통과를 켜면 위젯은 마우스 입력을 받지 않으며, 트레이 메뉴와 설정에서 해제할 수 있습니다.
Codex 모듈은 정상 상태 문구를 숨기고 조회 실패·로그인 필요·이전 데이터처럼 확인이 필요한 상태를 별도 줄로 표시합니다. Claude·Cursor도 정상 서버 조회값이면 하단 **업데이트됨** 행과 여백을 모두 접습니다. 마지막 확인·수신 시각과 출처 설명은 모듈 툴팁·상세 팝업에 유지합니다. Claude의 Desktop 로컬 기록·Code statusLine 수신값은 **수신됨**을 표시하며, 오래된 데이터·조회 실패·로그인/계정 변경·최초 수신 대기 상태도 별도 행으로 남깁니다. 오류가 나면 행이 다시 나타나고 정상 서버 조회에 성공하면 다시 접히며, 한도 표시·링 정렬·위젯 폭은 유지합니다. 리셋은 "35분 후"처럼 표시하고 정확한 날짜·시각은 툴팁에서 확인합니다. 리셋 시각이 제공되지 않으면 **리셋 미제공**으로, 이미 지난 시각은 **갱신 대기**로 표시하며 로컬에서 새 주기를 추정하지 않습니다.
...
위젯이나 상세 팝업을 화면 가장자리 근처에 내려놓으면 크기는 유지한 채 2 DIP 안쪽으로 정렬합니다. **Shift를 누른 채 놓으면** 이번 이동의 정렬을 생략합니다. 붙인 뒤 확대·축소하거나 내용 높이가 바뀌어도 해당 여백을 유지합니다. **설정 → 일반 → 화면 가장자리에 자동 정렬**에서 끌 수 있습니다.
...
세 제공자 모두 위젯은 정수 중심으로, 상세 팝업의 링·계정 요약·한도별 행은 소수점 최대 두 자리로 표시합니다. 위젯은 원본에서 직접 반올림하여 **76.91% → 77%**, **76.499% → 76%**로 표시합니다. 같은 한도의 사용·잔여가 보완 관계이면 잔여 표시는 반올림한 사용량의 보완값으로 맞춰 **76.5% / 23.5% → 77% / 23%**가 됩니다. 0보다 크고 1%보다 작으면 **<1%**, 99%보다 크고 100%보다 작으면 **>99%**로 표시하며 정확한 0%·100%는 그대로 유지합니다. 상세는 **97.1%**, **77%**처럼 불필요한 끝자리 0을 생략하고, 아주 작은 잔여·사용량도 **<0.01%**, **>99.99%**로 구분합니다. 미확인·비정상 퍼센트는 **?**로 표시합니다. 금액·무제한·꺼짐, 기존 위젯 툴팁의 정밀한 값, 원본 수치·링 진행률·경고 색상은 유지합니다. 트레이의 숫자·툴팁·링 정책도 유지하며 Cursor 트레이 숫자는 계속 **76.91% → 77**로 표시합니다.

트레이의 **사용률 숫자**는 투명 배경에 글꼴의 원래 비율을 유지한 굵은 숫자를 아이콘 공간에 맞춰 최대한 크게 표시합니다. **%** 기호는 생략하며, **67**은 **67% 사용**을 뜻합니다. 앱 테마와 별개로 Windows 작업표시줄이 어두우면 흰색, 밝으면 어두운 글자를 사용합니다. 선택한 계정의 사용률이며, 상세 카드에서 고른 표시 기간을 따릅니다. **자동**은 확인 가능한 5시간 값을 우선하고, 없으면 주간 값을 사용합니다. 실제 표시 중인 기간은 툴팁에서 확인할 수 있습니다. 기간 전환은 이미 받은 값의 표시만 바꾸며 수신 시각이나 사용량을 갱신하지 않습니다. 알 수 없는 값은 **?**로 표시합니다. 상태와 수신 시각은 툴팁이나 상세 카드에서 확인할 수 있습니다. 선택 가능한 **사용률 링**은 정상 수신값을 반올림 전 실제 사용률로 구분합니다. 70% 미만은 파랑, 70% 이상은 앰버, 85% 이상은 주황, 100% 이상은 빨강입니다. 99.6%처럼 숫자가 **100**으로 보여도 주황으로 표시하며 소진으로 처리하지 않습니다. 이전 데이터 또는 조회·연결 문제는 사용률과 관계없이 기존 노랑을 유지합니다. 알 수 있는 값이 없으면 회색 링에 **?**를 표시합니다.

<p><img src="images/ring-bands-widget-ko-dark.png" alt="가상 계정 8개를 보여주는 다크 위젯: Personal 69.99% 파랑, Research 70%와 Design 84.99% 앰버, Work 85%와 Writing 99.6% 주황, Prototype 100% 빨강, 이전 데이터 Travel 90% 노랑, 알 수 없는 Lab 회색" width="900"></p>
<p><img src="images/ring-bands-detail-ko-light.png" alt="69.99% 파랑, 70% 앰버, 원을 다 채우지 않는 99.6% 주황, 100% 빨강을 보여주는 라이트 상세 팝업" width="900"></p>
<p><img src="images/ring-bands-tray.png" alt="어두운·밝은 작업표시줄의 16·24·32픽셀 사용률 링 트레이 아이콘: 69.99, 70, 84.99, 85, 99.6, 100, 이전 데이터 90, 알 수 없음" width="560"></p>

*가상 샘플 계정으로 렌더링한 사용률 링 색상입니다. Codex 트레이 숫자는 99.6%를 100으로 반올림하지만 링은 주황입니다.*
...
설정·계정·캐시는 기존 `%LOCALAPPDATA%\ProMeter`에 유지합니다. 안정 설치의 실제 앱은 관리형 설치 루트 아래 `current\CycleArc.exe`이며, 새 설치의 기본 루트는 `%LOCALAPPDATA%\Programs\CycleArc`, 기존 설치는 `%LOCALAPPDATA%\CycleArc` 같은 등록된 위치를 유지합니다. 루트의 `CycleArc.exe`는 데스크톱과 Windows 시작용 안정 런처입니다. Claude 콜백은 stdin/stdout 전달을 위해 current 실행 파일을 직접 사용하며, 일치하는 CLI 신원과 저장된 바인딩을 확인한 뒤 기존 소유 wrapper를 옮깁니다. 확인된 이전 트레이 항목은 새 트레이 아이콘이 준비된 뒤 레지스트리 값을 백업하고 정리합니다.
