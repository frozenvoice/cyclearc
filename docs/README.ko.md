...
**연결 상세 설정 → 연결 해제**는 해제 상태를 먼저 저장한 뒤 기존 상태 표시줄을 복원하고 CycleArc의 오류 수신 훅을 제거합니다. 설정 파일 정리에 실패해도 연결은 해제되며, 정리가 완료되지 않았다는 안내를 표시합니다. 메인 화면의 숨김 상태는 재시작해도 유지됩니다. 계정 관리의 프로필, 마지막 정상 캐시와 설정 위치는 남기며 Claude 자체를 로그아웃하지 않습니다. 다시 연결하면 메인에 **수신 대기**로 표시하고 새 공식 사용량을 기다립니다.
...
Release 빌드, 전체 테스트와 화면 검증을 수행하고 디버그 심볼을 제외한 Windows x64 단일 파일을 설치·실행합니다. 연결된 Git 작업 폴더들은 원본 작업 폴더의 `publish/local` 설치를 함께 사용하므로 개발 실행 위치가 바뀌어도 트레이 항목이 늘어나지 않습니다. Git 정보가 없는 소스 사본은 자체 `publish/local`을 사용합니다. 빌드 임시 파일은 현재 작업 폴더에 남으며 `-NoLaunch`는 설치된 앱을 교체하지 않습니다.
설치 파일 잠금은 최대 30회 재시도하며, 교체나 시작 실패 시 이전 설치를 복원하고 다시 실행합니다.
시작 시 실행 중인 앱의 PID와 경로를 표시합니다. 검증 후에는 현재 Windows 세션에서 확인된 CycleArc 데스크톱 앱을 종료하고, 중복 실행 잠금이 해제된 것을 확인한 뒤 파일을 교체합니다. 빌드 산출물에서 직접 실행 중인 앱은 정리 전에 경로와 PID를 알려주므로, 해당 앱을 종료한 뒤 다시 실행하세요.
