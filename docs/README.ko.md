...
Cursor 모델(Auto)과 외부 모델(API) 한도는 각각 표시합니다. 제공되는 추가 사용 예산과 별도 Grok 한도도 섞지 않습니다. API에 없는 한도·리셋 시각은 미확인으로 유지하며, 예전 달러 사용액을 모델별 퍼센트와 합산하지 않습니다. 조회 실패 시 마지막 정상값과 확인 시각을 오래된 데이터로 유지합니다. Cursor 로그인 계정이 바뀌면 기존 계정의 잔여량은 숨깁니다. 해당 프로필에 다시 연결하려면 Cursor에서 원래 계정으로 로그인하세요.

Grok만 확인할 수 없으면 월간 한도는 정상 갱신 상태로 계속 표시하고 조회합니다. Grok의 재시도 대기는 Grok에만 적용하며, 확인하지 못한 값은 생략하고 팝업에 이유를 안내합니다. 계정 불일치 상태에서도 팝업을 다시 열거나 CycleArc를 재시작할 때 설정된 자동 확인 주기를 지킵니다.
