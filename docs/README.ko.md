...
**Codex는 확인한 계정의 연결을 유지합니다.** 연결된 Codex의 로그인이 바뀌면 **로그인 변경 감지**를 표시하고 기존 별명 아래 사용량을 표시하지 않습니다. 별도로 로그인한 다른 프로필과 같은 로그인 이메일이 조회되면 **연결 확인 필요**를 표시합니다. **계정 관리 → 다시 연결**에서 원래 계정으로 공식 브라우저 로그인을 완료하세요. 사용량까지 확인한 뒤 독립된 로그인으로 교체하며 별명·순서·선택 상태는 유지합니다. 취소하거나 이미 연결된 계정을 고르거나 사용량 조회가 실패하면 기존 연결을 유지합니다. 공식 로그인 정보만으로는 같은 이메일을 쓰는 서로 다른 워크스페이스를 구분할 수 없습니다.
...
- **다시 로그인**: CycleArc에서 추가한 Codex 프로필의 로그인을 갱신하거나 변경합니다. 기존 Codex에서 연결한 프로필은 원래 Codex에서 다시 로그인하세요. Claude 프로필은 **연결** 버튼을 사용합니다.
