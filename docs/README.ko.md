...
트레이의 **사용률 숫자**는 Windows 아이콘 공간에 큰 굵은 숫자로 표시합니다. 선택한 계정의 사용률이며, 주간 값이 있으면 주간 기준, 없으면 5시간 기준입니다. **파랑**은 정상 데이터(기존 값을 유지한 정상 새로고침 포함), **빨강**은 정상 수신값의 사용률 100%, **노랑**은 이전 데이터 또는 조회·연결 문제, **회색 ?**는 사용량 수신 대기입니다. 정확한 상태와 수신 시각은 상세 카드에서 확인할 수 있습니다. 설정에서 **사용률 링**도 선택할 수 있습니다.
