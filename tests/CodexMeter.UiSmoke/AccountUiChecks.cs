...
internal static class AccountUiChecks
{
    public static void Run(string? directory = null)
    {
        ...
        CheckLocalIcons();
        Console.WriteLine($"PASS: {widgetIdentityCount} widget account identity renders; single account, nickname/email fallback, quota states, long email and removal.");
    ...
    private static int CheckWidgetAccountIdentity(string? directory, UiLanguage language, AppTheme theme)
    {
        ...
        try
        {
            foreach (var item in cases)
            {
                ...
                var badge = (CodexProviderBadge)widget.FindName("ProviderBadge");
