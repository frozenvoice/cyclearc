...
public sealed record DesktopLaunchOptions(bool Replace, bool Autorun, bool Show, bool Quiet,
    bool StatusOnly, string? ExpectedHash, string? ExpectedVersion)
{
    public static DesktopLaunchOptions Parse(IReadOnlyList<string> args)
    {
        bool replace = false, autorun = false, show = false, quiet = false, status = false;
        ...
        if ((replace && autorun) || (status && (replace || autorun || show))
        ...
        return new(replace, autorun, show, quiet, status, hash, version);
    }
